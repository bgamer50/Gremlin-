...
class ReferenceVertex: public Vertex {
    ...
    public:
    	...
    	virtual std::vector<Edge*> edges(Direction dir) { throw std::runtime_error(STR_NOT_SUPPORTED); }

    	virtual Property* property(std::string key) { throw std::runtime_error(STR_NOT_SUPPORTED); }

    	virtual Property* property(Cardinality cardinality, std::string key, boost::any& value) { throw std::runtime_error(STR_NOT_SUPPORTED); }
